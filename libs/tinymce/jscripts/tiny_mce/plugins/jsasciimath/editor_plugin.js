...
(function() {
	...
	tinymce.PluginManager.requireLangPack('jsasciimath');

	tinymce.create('tinymce.plugins.jsjsAsciimathPlugin', {
		/**
		...
		init : function(ed, url) {
			...
			// Register the command so that it can be invoked by using tinyMCE.activeEditor.execCommand('mcejsAsciimath');
			ed.addCommand('mcejsAsciimath', function(val) {
			...
			ed.addCommand('mcejsAsciimathDlg', function() {
				...
				ed.windowManager.open({
					...
					height : 140 + parseInt(ed.getLang('jsasciimathdlg.delta_height', 0)),
					inline : 1
				}, {
			...
			// Register jsasciimath button
			ed.addButton('jsasciimath', {
				title : 'jsasciimath.desc',
				cmd : 'mcejsAsciimath',
				image : url + '/img/ed_mathformula2.gif'
			});
			
			ed.addButton('jsasciimathcharmap', {
				title : 'jsasciimathcharmap.desc',
				cmd : 'mcejsAsciimathDlg',
			...
			*/
			...
			ed.onBeforeExecCommand.add(function(ed,cmd) {
				if (cmd != 'mcejsAsciimath' && cmd != 'mcejsAsciimathDlg') {
					AMtags = ed.dom.select('span.AM');
					for (var i=0; i<AMtags.length; i++) {
						t.math2ascii(AMtags[i]);
	...
	// Register plugin
	tinymce.PluginManager.add('jsasciimath', tinymce.plugins.jsjsAsciimathPlugin);
})();
